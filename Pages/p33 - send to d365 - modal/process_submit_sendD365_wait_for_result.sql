...
-- [UNCOMPLETED]
-- [TO-DO]:
-- - return ID of the LOGS_API_RESPONSE in the procedure SP_UPDATE_TIME_SHEET
-- - check the API status in table LOGS_API_RESPONSE with ID returned from the procedure SP_UPDATE_TIME_SHEET
...
declare
    ...
    v_delay NUMBER := 5; -- Delay in seconds for inner loop
    v_max_wait_time NUMBER := 60; -- maximum wait time in seconds
begin
    if trim(:P33_EMPLOYEE_NAME) = 'all' then    -- case when select all employee
        ...
        loop
            ...
            -- Polling Mechanism to wait for the result from Procedure

            v_start_time := DBMS_UTILITY.GET_TIME;

            LOOP
                SELECT RESPONSE_STATUS INTO v_status FROM LOGS_API_RESPONSE WHERE NOTE LIKE '%' || rec.TS_HEADER_ID || '%';

                EXIT WHEN v_status = 1 OR (DBMS_UTILITY.GET_TIME - v_start_time) > (v_max_wait_time * 100);

                v_start_time := DBMS_UTILITY.GET_TIME; -- Reset start time for inner loop (delay)

                LOOP
                    v_current_time := DBMS_UTILITY.GET_TIME;
                    EXIT WHEN (v_current_time - v_start_time) > (v_delay * 100);
                END LOOP;
            END LOOP;
