...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_APPROVED_MANAGER > List of Values > SQL Query

SELECT FULL_NAME,EMPLOYEE_CODE as ID FROM EMPLOYEES WHERE POSITION_ID = (
select MANAGER_POSITION_ID FROM EMPLOYEES WHERE EMPLOYEE_CODE = :APP_EMP_CODE );

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Process: Calculate request > Source > PL/SQL Code
        ...
        FROM
            ...
            JOIN (
                ...
                WHERE
                    ...
                    AND ID = TO_NUMBER(:P10_REQUEST_ID_IMPORTED)
            ) ER
            ON ER.EMPLOYEE_CODE_REQ = EAL.EMPLOYEE_CODE
        WHERE
            EAL.EMPLOYEE_CODE = :APP_EMP_CODE
            AND EXPIRATION_DATE >= TO_CHAR(SYSDATE, 'MM/DD/YYYY')
            AND EAL.HRM_ABSENCE_CODE_GROUP_ID = :P10_ANNUAL_LEAVE
    )LOOP
        IF REC.HRM_ABSENCE_CODE_GROUP_ID = 'APL' THEN
            UPDATE ABSENCE_GROUP_EMPLOYEE
            SET
                CARRY_FORWARD_USED = REC.CARRY_FORWARD_USED + REC.CRF_DAY_TEMP,
                DAY_APPROVE = SYSDATE,
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Process: Calculate request > Server-side Condition (Function Body) > PL/SQL Function Body
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Process: Save when submit > Source > PL/SQL Code
    ...
    --     -- end loop;
    --     v_annual_temp := :P10_TOTAL_DAYS;
    --     v_benefit_code := :P10_LEAVE_DETAIL;
    -- end if;
    ...
    --         :P10_START_TIME,:P10_END_TIME,sysdate,v_benefit_code,v_crf_temp,v_annual_temp,nvl(:P10_NAME_FILES,''),nvl(:P10_URL_FILES,'#'), :P10_ANNUAL_LEAVE_BALANCE
...
begin
    ...
    requester_schedule := LOWER(TRIM(:P10_LEAVE_TYPE_DEPT));

    -- Waiting for the end to come
    -- dbms_session.sleep(1);

    -- INSERT INTO DEMO_ATTACHMENT(URL) VALUES(:P10_URL_FILES);
    ...
    -- VALUES (TO_DATE(:P10_FROM_DATE, 'DD/MM/YYYY'),TO_DATE(:P10_END_DATE, 'DD/MM/YYYY'), :P10_FROM_DATE, :P10_END_DATE);
    ...
    -- '<p> Employee Code: '|| :P10_EMPLOYEE_CODE ||' </p>' ||
    -- '<p> Total Days: '|| CASE
    --            WHEN :P10_TOTAL_DAYS = 0.5 THEN to_char(:P10_TOTAL_DAYS, '0.0')
    --            ELSE :P10_TOTAL_DAYS
    --        END ||' </p>' ||
    -- '<p> From Date: '|| :P10_FROM_DATE ||' </p>' ||
    -- '<p> To Date: '|| :P10_END_DATE ||' </p>' ||
    ...
    v_body_emp := v_body_emp || '<p style=''color:black;margin-top:20px''>Anh/Chị '|| :P10_EMPLOYEE ||' thân mến,</p>';
    v_body_emp := v_body_emp || '<p style=''color:black;margin-top:0''>Dear Mr/Ms. '|| :P10_EMPLOYEE ||',</p>';
    ...
    v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Họ và tên/ Full name:</strong> '|| :P10_EMPLOYEE ||'</p>';
    v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Mã số nhân viên/ Employee Code:</strong> '|| :P10_EMPLOYEE_CODE ||'</p>';
    v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Loại phép/ Leave Type:</strong> '|| :P10_ANNUAL_LEAVE ||'</p>';
    if (:P10_TOTAL_DAYS <= 0.5) then
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Tổng Số Ngày/ Total Days:</strong> '|| to_char(:P10_TOTAL_DAYS, '0.0') ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Từ Ngày/ From Date:</strong> '|| :P10_FROM_DATE ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Đến Ngày/ To Date:</strong> '|| :P10_END_DATE ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Từ/ Start Time:</strong> '|| :P10_START_TIME ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Đến/ To Time:</strong> '|| :P10_END_TIME ||'</p>';
    else
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Tổng Số Ngày/ Total Days:</strong> '|| :P10_TOTAL_DAYS ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Từ Ngày/ From Date:</strong> '|| :P10_FROM_DATE ||'</p>';
        v_body_emp := v_body_emp || '<p style=''color:black''><strong style=''color:black''>- Đến Ngày/ To Date:</strong> '|| :P10_END_DATE ||'</p>';
    end if;
    ...
    --     '<p>A leave request from '|| :P10_EMPLOYEE ||' requires your approval. Here are the details:</p>' ||
    --     '<p>Employee Code: '|| :P10_EMPLOYEE_CODE ||'</p>' ||
    --     -- '<p>Total Days: '|| LPAD(:P10_TOTAL_DAYS, 3, '0') ||'</p>' ||
    --     '<p>Total Days: '|| CASE
    --            WHEN :P10_TOTAL_DAYS = 0.5 THEN to_char(:P10_TOTAL_DAYS, '0.0')
    --            ELSE :P10_TOTAL_DAYS
    --        END ||'</p>' ||
    ...
    v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Họ và tên/ Full name:</strong> '|| :P10_EMPLOYEE ||'</p>';
    v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Mã số nhân viên/ Employee Code:</strong> '|| :P10_EMPLOYEE_CODE ||'</p>';
    v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Loại phép/ Leave Type:</strong> '|| :P10_ANNUAL_LEAVE ||'</p>';
    if (:P10_TOTAL_DAYS <= 0.5) then
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Tổng Số Ngày/ Total Days:</strong> '|| to_char(:P10_TOTAL_DAYS, '0.0') ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Từ Ngày/ From Date:</strong> '|| :P10_FROM_DATE ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Đến Ngày/ To Date:</strong> '|| :P10_END_DATE ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Từ/ Start Time:</strong> '|| :P10_START_TIME ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Đến/ To Time:</strong> '|| :P10_END_TIME ||'</p>';
    else
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Tổng Số Ngày/ Total Days:</strong> '|| :P10_TOTAL_DAYS ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Từ Ngày/ From Date:</strong> '|| :P10_FROM_DATE ||'</p>';
        v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Đến Ngày/ To Date:</strong> '|| :P10_END_DATE ||'</p>';
    end if;
    v_body_man := v_body_man || '<p style=''color:black''><strong style=''color:black''>- Ghi Chú/ Note:</strong> '|| :P10_NOTE ||'</p>';
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Validation: Unvalid benefit absence group > Validation > PL/SQL Function Body
...
BEGIN
    ...
    where EMPLOYEE_CODE = :APP_EMP_CODE and :P10_LEAVE_DETAIL = HRM_ABSENCE_CODE_ID;
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Validation: Check 23rd day > Validation > PL/SQL Function Body
...
BEGIN
    ...
    IF(to_date(:P10_FROM_DATE,'DD/MM/YYYY') >= date_23_last_month) THEN
        RETURN TRUE;
    ELSE RETURN FALSE;
    END IF;
END;
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Validation: Check overlapping time > Validation > PL/SQL Function Body
...
BEGIN
    INP_START_TIME := TRIM(:P10_START_TIME);
    INP_END_TIME := TRIM(:P10_END_TIME);
    ...
    FOR rec IN (SELECT start_time, end_time 
                ...
                AND :P10_ALL_DAY = 'N' 
                AND TO_DATE(:P10_FROM_DATE, 'DD/MM/YYYY') = FROM_DATE)
    LOOP
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Validation: Check overlapping leaves > Validation > PL/SQL Function Body
...
DECLARE
    ...
    flag BOOLEAN := true;
    p10_total_days NUMBER;
BEGIN
    p10_total_days := TO_NUMBER(:P10_TOTAL_DAYS);
    
    IF(:P10_ALL_DAY = 'N') THEN
        :P10_END_DATE := :P10_FROM_DATE;
        p10_total_days := 0.5;
    END IF;
    ...
    FOR REC IN (
        ...
        WHERE
            ...
            AND (TO_DATE(:P10_END_DATE, 'DD/MM/YYYY') >= TO_DATE(ER.FROM_DATE, 'MM/DD/YYYY') 
            AND TO_DATE(:P10_FROM_DATE, 'DD/MM/YYYY') <= TO_DATE(ER.END_DATE, 'MM/DD/YYYY'))
    ) LOOP
    ...
    IF TOTAL_OVERLAP_DAYS >= 1 
        OR (TO_NUMBER(p10_total_days) = 0.5 AND TOTAL_OVERLAP_DAYS > 0.5)
        OR (TOTAL_OVERLAP_DAYS = 0.5 AND TO_NUMBER(p10_total_days) >= 1 ) THEN
        flag := FALSE;
    ELSE
        flag := TRUE;
    END IF;
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Validation: Check total > Validation > PL/SQL Function Body
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: On change > Action: Set End_time_HO > Settings > PL/SQL Code

BEGIN
    :P10_END_TIME := CASE
        WHEN TRIM(:P10_START_TIME_HO) = '08:00' THEN '12:00'
        WHEN TRIM(:P10_START_TIME_HO) = '13:30' THEN '18:00'
    END;
END;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Change Type > Action: Execute Server-side Code > Settings > PL/SQL Code
...
where USER_NAME = :APP_USER_NAME and EXPIRATION_DATE >= to_char(sysdate,'MM/DD/YYYY') 
 and HRM_ABSENCE_CODE_GROUP_ID = :P10_ANNUAL_LEAVE)loop
    if rec.CARRY_FORWORD_EXP_DATE >= to_char(sysdate,'MM/DD/YYYY')  then
        :P10_ANNUAL_LEAVE_BALANCE := rec.AVAILABLE + rec.CARRY_FORWARD_AVALABLE;
    else
        :P10_ANNUAL_LEAVE_BALANCE := rec.AVAILABLE;
    end if;
end loop;
/*
select nvl(AVAILABLE+CARRY_FORWARD_AVALABLE,0) into :P10_ANNUAL_LEAVE_BALANCE
from ABSENCE_GROUP_EMPLOYEE emp_al
right JOIN EMPLOYEES e ON emp_al.EMPLOYEE_CODE = e.EMPLOYEE_CODE
where e.USER_NAME = :APP_USER_NAME and HRM_ABSENCE_CODE_GROUP_ID = :P10_ANNUAL_LEAVE and EXPIRATION_DATE >= to_char(sysdate,'MM/DD/YYYY');
*/
end;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Change Type > Action: Update Leave Detail list > Settings > SQL Statement

select ACL_DESCRIPTION, ACL_ABSENCECODE_ID as ID 
FROM ABSENCE_CODE_LIST
WHERE ABSENCE_GROUP = :P10_ANNUAL_LEAVE
ORDER BY ACL_DESCRIPTION;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Change from date > Action: Execute Server-side Code > Settings > PL/SQL Code

begin
if :P10_ALL_DAY = 'Y' then
select ROUND(TO_DATE(:P10_END_DATE,'DD/MM/YYYY')-TO_DATE(:P10_FROM_DATE,'DD/MM/YYYY'))+1 into :P10_TOTAL_DAYS FROM DUAL;
end if;
if :P10_ALL_DAY = 'N' then
select (TO_DATE(:P10_END_DATE,'DD/MM/YYYY')-TO_DATE(:P10_FROM_DATE,'DD/MM/YYYY'))+0.5 into :P10_TOTAL_DAYS FROM DUAL;
end if;
end;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Change from date > Action: Change End_Date when it disabled > Settings > PL/SQL Code

BEGIN
    :P10_END_DATE := :P10_START_DATE;
END;


-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Set display time > Action: Calculate Total Days > Settings > PL/SQL Code
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: Change end date > Action: Execute Server-side Code > Settings > PL/SQL Code
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: ON_CHANGE_P10_LEAVE_DETAIL, ON_CHANGE_FROM_DATE > Action: Change end_Date > Settings > PL/SQL Function Body
...
begin
    ...
    -- SELECT MAXIMUM_LIMIT_PER_TIME INTO v_total_maxium_date FROM ABSENCE_CODE_LIST WHERE lower(ACL_ABSENCECODE_ID) = lower(:P10_LEAVE_DETAIL) AND ROWNUM=1;
    -- return TO_CHAR((to_date(:P10_FROM_DATE,'DD/MM/YYYY') + INTERVAL '1' DAY * (FLOOR(abs(v_total_maxium_date)) - 1)),'DD/MM/YYYY');

    SELECT MAXIMUM_MONTHS_LIMIT_PER_TIME into total_month FROM ABSENCE_CODE_LIST WHERE lower(ACL_ABSENCECODE_ID) = lower(:P10_LEAVE_DETAIL) AND ROWNUM=1;
    -- DBMS_OUTPUT.PUT_LINE(total_month);
    o_end_date := to_date(:P10_FROM_DATE, 'DD/MM/YYYY') + interval '1' month * total_month - interval '1' day;
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Dynamic Action: ON_CHANGE_P10_LEAVE_DETAIL, ON_CHANGE_FROM_DATE > Action: Change End_Date when in specified absence_code > Settings > PL/SQL Function Body
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_TOTAL_DAYS > Default > SQL Query

SELECT 
--case :P10_ALL_DAY
--when 'Y' then
--TO_DATE(:P10_END_DATE,'MM/DD/YYYY')-TO_DATE(:P10_FROM_DATE,'MM/DD/YYYY') 
--else
--TO_DATE(:P10_END_DATE,'MM/DD/YYYY')-TO_DATE(:P10_FROM_DATE,'MM/DD/YYYY')+0.5
--end as total
0
FROM DUAL;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_END_DATE > Default > SQL Query

select TO_DATE(sysdate, 'MM/DD/YYYY') from dual;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_MANAGER_CODE > Default > SQL Query

SELECT manager_position_id FROM EMPLOYEES WHERE EMPLOYEE_CODE = :APP_EMP_CODE;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_LEAVE_TYPE_DEPT > Default > SQL Query

select SCHEDULE from employees where EMPLOYEE_CODE = :APP_EMP_CODE;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_FROM_DATE > Default > SQL Query

select TO_DATE(sysdate, 'MM/DD/YYYY') from dual;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_CURRENT_TIME_PROFILE > Default > SQL Query

select PROFILE_ID from employees where EMPLOYEE_CODE = :APP_EMP_CODE;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_ANNUAL_LEAVE > List of Values > SQL Query
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_LEAVE_DETAIL > List of Values > SQL Query
...
-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_EMPLOYEE > Default > SQL Query

select full_name from employees where EMPLOYEE_CODE = :APP_EMP_CODE;

-- ----------------------------------------
-- Page: 10 - Leave Request for Manager > Page Item: P10_APPROVED_MANAGER > Default > SQL Query

SELECT EMPLOYEE_CODE as ID FROM EMPLOYEES WHERE POSITION_ID = (
select MANAGER_POSITION_ID FROM EMPLOYEES WHERE EMPLOYEE_CODE = :APP_EMP_CODE );
