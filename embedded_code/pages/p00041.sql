...
-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Region: Report > Source > SQL Query
...
from
    ...
    join WORKFLOW_APPROVAL wa on wa.WF_PROCESS_ID = er.WF_PROCESS_ID
    join employees emp on emp.employee_code = er.EMPLOYEE_CODE
where er.next_step > 1 and wa.WA_GROUP in (select WA_GROUP from temp)
and lower(emp.FULL_NAME) like '%' || TRIM(lower(:P41_EMPLOYEE)) ||'%'
and to_date(er.CREATE_DATE,'MM/DD/YYYY') between to_date(:P41_FROM_DATE,'MM/DD/YYYY') and to_date(:P41_TO_DATE,'MM/DD/YYYY')
and lower(er.STATUS) like '%' || :P41_STATUS || '%';

-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Region: Report > Column: STATUS > List of Values > SQL Query

SELECT STATUS, ID AS ID 
From STATUS;

-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Dynamic Action: Onclick > Action: Reset page items > Settings > PL/SQL Code
...
-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Page Item: P41_TO_DATE > Default > SQL Query

SELECT TO_CHAR(SYSDATE, 'MM/DD/YYYY') AS formatted_date FROM DUAL;

-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Page Item: P41_FROM_DATE > Default > SQL Query

SELECT TRUNC(SYSDATE, 'YEAR') AS first_day_of_current_year FROM DUAL;

-- ----------------------------------------
-- Page: 41 - Overtime Request Approval > Page Item: P41_STATUS > List of Values > SQL Query
