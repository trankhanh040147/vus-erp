...
n_wt_in1 varchar2(20);
n_wt_out1 varchar2(20);
n_wt_in2 varchar2(20);
n_wt_out2 varchar2(20);
n_wt_in1_line_id varchar2(100);
n_wt_out1_line_id varchar2(100);
n_wt_in2_line_id varchar2(100);
n_wt_out2_line_id varchar2(100);
...
BEGIN
    ...
    LOOP
        ...
        SELECT COUNT(*) into n_count_1 FROM P_TIME_SHEET_DETAIL ptsd WHERE trim(ptsd.TS_HEADER_ID) = trim(rec_ets.TS_HEADER_REC_ID);
        ...
        DBMS_OUTPUT.put_line('Header ID:' || rec_ets.TS_HEADER_REC_ID);
        ...
        if n_count_1 > 0 then
            ...
            SET
                WT_STANDARD_HOURS = rec_ets.PROFILE_PAID_HOURS,
        ...
        -- [Not tested yet]
        -- Get (WT_IN1,WT_OUT1,WT_IN2,WT_OUT2) from EMP_TIME_SHEET_LINE
        
        -- init null variables
        n_current_in := 1;
        n_current_out := 1;
        n_wt_in1_line_id := NULL;
        n_wt_out1_line_id := NULL;
        n_wt_in2_line_id := NULL;
        n_wt_out2_line_id := NULL;  
        n_wt_in1 := NULL;
        n_wt_out1 := NULL;
        n_wt_in2 := NULL;
        n_wt_out2 := NULL;
        ...
        for rec_etsl in (SELECT * FROM EMP_TIME_SHEET_LINE etsl where etsl.TS_HEADER_REC_ID = rec_ets.TS_HEADER_REC_ID order by TO_DATE_HH24MI(etsl.START_TIME))
        LOOP
            if rec_etsl.reg_type like '%clock in%' then
                -- output for testing
                DBMS_OUTPUT.put_line('Line ID:' || rec_etsl.TS_LINE_REC_ID || ' ' || rec_etsl.reg_type || ' ' || rec_etsl.start_time);
        ...
        SET
            ...
            ptsd.AT_IN1 = CASE WHEN ptsd.AT_IN1 IS  NULL THEN n_wt_in1 ELSE ptsd.AT_IN1 END,
            ptsd.AT_OUT1 = CASE WHEN ptsd.AT_OUT1 IS  NULL THEN n_wt_out1 ELSE ptsd.AT_OUT1 END,
            ptsd.AT_IN2 = CASE WHEN ptsd.AT_IN2 IS  NULL THEN n_wt_in2 ELSE ptsd.AT_IN2 END,
            ptsd.AT_OUT2 = CASE WHEN ptsd.AT_OUT2 IS  NULL THEN n_wt_out2 ELSE ptsd.AT_OUT2 END
        WHERE ptsd.TS_HEADER_ID = rec_ets.TS_HEADER_REC_ID;

    END LOOP;

    SP_TIMESHEET_CALCULATE_WORKING_TIME();
